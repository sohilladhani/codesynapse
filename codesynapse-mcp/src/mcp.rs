...
STEP 1 — ALWAYS call codesynapse_context FIRST for ANY of these:\n\
  ...
  If result does NOT start with '[No exact match' → answer directly from it.\n\
\n\
...
STEP 2b — If exploring a multi-component mechanism (scheduler, I/O pipeline, runtime):\n\
  If your context results are ALL from the SAME directory or file after 2-3 calls,\n\
  you are seeing ONE layer — pivot and search adjacent components:\n\
    e.g. found sync/waker → also search runtime/scheduler and runtime/task\n\
    e.g. found middleware → also search router and handler\n\
