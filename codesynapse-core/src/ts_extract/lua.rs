...
impl TsLuaExtractor {
    pub fn extract(source: &[u8], path: &Path) -> Result<ExtractionFragment> {
        ...
        "#;
        if let Ok(mut captures) = run_query_named(source, &lang, require_query) {
            for path_str in captures.remove("require.path").unwrap_or_default() {
                let module = path_str.trim_matches('"').trim().trim_matches('\'');
...
impl LanguageExtractor for TsLuaExtractor {
